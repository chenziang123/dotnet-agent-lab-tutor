...
| `GetDocSection` | 展开指定文档片段 | 调用`IRagService.GetChunkAsync`，适合追问某个`chunkId` |
| `ListTopics` | 列出知识库主题 | 返回8个课程文档覆盖范围 |
| `OpenPage` | 打开GUI页面 | 通过Playwright打开`http`、`https`或`file`页面 |
...
DotNetLabTutor.Tools.Tests：11个通过
总计27个通过
```
